...
include('conn.php');
$a=$_SESSION["username"];
$query3 = "select username from user where user.username='$a'";
...
if(isset($_POST['Submit'])) {
    ...
    $query = "select * from user where user.password='$b'";
    $result = mysql_query($query, $conn);
    if (mysql_num_rows($result)== 0) {
        echo"<script>alert('原密码不正确')</script>";
    }else{
        $password2=$_POST["password2"];
        $sql="update user set password='$password2' WHERE user.username='$a'";
        $result2 = mysql_query($sql, $conn);
        echo "<script>alert('修改成功,请重新进行登陆！');window.location='admin_management.php'</script>";
        exit();
    }
}
?>
...
<table cellpadding="3" cellspacing="1" border="0" width="100%" class="table" align=center>
    <form name="renpassword" method="post" action="">
        <tr>
            <th height=25 colspan=4 align="center" class="bg_tr">修改密码</th>
        </tr>
        <tr>
        ...
        <tr>
            <td colspan="2" align="center" class="td_bg">
                <input class="button" onClick="return check();" type="submit" name="Submit" value="修改">
            </td>
        </tr>
    </form>
</table>
