...
<head>
    <?php
    include('conn.php');
    $arr=mysql_query("select * from book where book_id='".$_GET['id']."'",$conn);
    $rows=mysql_fetch_row($arr);
    ?>
    <?php
    if(isset($_POST['button'])){
        $sqlstr = "update book set  pic = '".$_POST['pic']."',book_title = '".$_POST['book_title']."',author = '".$_POST['author']."',publish_id= '".$_POST['publish_id']."', introduction = '".$_POST['introduction']."',type = '".$_POST['type']."',price= '".$_POST['price']."',total = '".$_POST['total']."'where book_id='".$_GET['id']."'";
...
<form id="myform" name="myform" method="post" action="" onSubmit="return myform_Validator(this)">
    <table width="100%" height="173" border="0" align="center" cellpadding="2" cellspacing="1" class="table">
        ...
        <tr>
            <td width="31%" align="right" class="td_bg">封面：</td>
            <td width="69%" class="td_bg">
                <input name="pic" type="text" id="pic" size="15" maxlength="30" />
            </td>
        </tr>
